  ...
  <div class="overflow-y-scroll">
    ...
    <div class="w-f flex flex-col px-4 py-2 bg-white">
      ...
      <div class="flex flex-row w-full border-b-2 border-primary-grey/20 mt-2 pb-2 px-2">
        ...
        <!--TODO CHANGE FOR ACTUAL IMG AFTER MERGING WITH PHOTO BRANCH)-->
        ...
        <div class="w-5/8 flex flex-col space-y-2 ml-2">
          ...
          <p class="text-base text-black font-medium">{{ thePost.description }}</p>
        </div>
      </div>
      <p class="text-base text-black border-b-2 border-primary-grey/20 py-4">{{ thePost.text }}</p>
...
import {LocationMarkerIcon, HeartIcon, AnnotationIcon, ThumbDownIcon, ThumbUpIcon, TrashIcon, PencilAltIcon, ChevronLeftIcon} from "@heroicons/vue/outline";
...
postService?.getPost(postId).then((result) => thePost.value = result.data);
